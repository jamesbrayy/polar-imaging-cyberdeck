...
colourlist = ["white", "cyan", "dark_blue", "dark_gray", "magenta"]
    ...
    if names == ["*"]:
        for i in range(0, len(lines), 3):
            ...
            if name not in used_names:
                sats.append(EarthSatellite(lines[i+1], lines[i+2], name))
                used_names.add(name)
    else:
        for name in names:
            found = False
...
class SatelliteApp:
    ...
    def create_metrics_table(self, sat_data):
        boxes = []
        for i, (name, az, el, lat, lon, alt, gc_dist, sl_dist, speed) in enumerate(sat_data):
    ...
    def create_main_widget(self):
        ...
        options_box = urwid.AttrMap(urwid.LineBox(urwid.Filler(options_text, valign='top'), title="Menu"), 'border')
        
        info = urwid.Filler(urwid.Pile([
            ('pack', status_box),
            ('pack', map_box),
            ('weight', 1, metrics_box),
        ]), valign='top')
        
        info = urwid.AttrMap(urwid.LineBox(info, title="Satellite Tracker"), 'border')
...
def get_user_input():
    names = [n.strip() for n in Prompt.ask("Enter satellites (comma-separated)", default="Meteor").split(",") if n.strip()][:5]
    coords = Prompt.ask("Observer coordinates", default="-31.9922 116.0934")
